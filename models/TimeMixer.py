    ...
    def __init__(self, args):
        ...
        self.down_sampling_layers = torch.nn.ModuleList(
            [
                nn.Sequential(
                    torch.nn.Linear(
                        args.seq_len // (args.down_sampling_window ** i),
                        args.seq_len // (args.down_sampling_window ** (i + 1)),
                    ),
                    nn.GELU(),
                    torch.nn.Linear(
                        args.seq_len // (args.down_sampling_window ** (i + 1)),
...
class PastDecomposableMixing(nn.Module):
    def __init__(self, args):
        ...
        if args.decomp_method == 'moving_avg':
            ...
            self.decompsition = DFT_series_decomp(args.top_k)
        else:
            raise ValueError('decompsition is error')

        if not args.channel_independence:
            self.cross_layer = nn.Sequential(
                nn.Linear(in_features=args.d_model, out_features=args.d_ff),
                nn.GELU(),
                nn.Linear(in_features=args.d_ff, out_features=args.d_model),
            )
...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.preprocess = series_decomp(args.moving_avg)
        self.enc_in = args.enc_in

        if self.channel_independence:
            self.enc_embedding = DataEmbedding_wo_pos(1, args.d_model, args.embed, args.freq,
                                                      args.dropout)
        else:
        ...
        self.normalize_layers = torch.nn.ModuleList(
            [
                Normalize(self.args.enc_in, affine=True, non_norm=True if args.use_norm == 0 else False)
    ...
    def __multi_scale_process_inputs(self, x_enc, x_mark_enc):
        if self.args.down_sampling_method == 'max':
            ...
            down_pool = nn.Conv1d(in_channels=self.args.enc_in, out_channels=self.args.enc_in,
        ...
        for i in range(self.args.down_sampling_layers):
            ...
            if x_mark_enc is not None:
                x_mark_sampling_list.append(x_mark_enc_mark_ori[:, ::self.args.down_sampling_window, :])
                x_mark_enc_mark_ori = x_mark_enc_mark_ori[:, ::self.args.down_sampling_window, :]
    ...
    def forward(self, batch_x, batch_x_mark, batch_y, batch_y_mark):
        ...
        batch_x, batch_x_mark = self.__multi_scale_process_inputs(batch_x, batch_x_mark)

        x_list = []
        x_mark_list = []
        if batch_x_mark is not None:
            for i, x, x_mark in zip(range(len(batch_x)), batch_x, batch_x_mark):
                B, T, N = x.size()
            ...
            for i, x in zip(range(len(batch_x)), batch_x, ):
                B, T, N = x.size()
                x = self.normalize_layers[i](x, 'norm')
                if self.channel_independence:
                    x = x.permute(0, 2, 1).contiguous().reshape(B * N, T, 1)
    ...
    def future_multi_mixing(self, B, enc_out_list, x_list):
        ...
        if self.channel_independence:
            ...
            for i, enc_out in zip(range(len(x_list)), enc_out_list):
                ...
                dec_out = self.projection_layer(dec_out)
                dec_out = dec_out.reshape(B, self.args.c_out, self.pred_len).permute(0, 2, 1).contiguous()
